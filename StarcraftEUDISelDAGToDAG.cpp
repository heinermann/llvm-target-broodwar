...
//===----------------------------------------------------------------------===//
//
// This file defines a DAG pattern matching instruction selector for StarcraftEUD,
// converting from a legalized dag to a StarcraftEUD dag.
//
//===----------------------------------------------------------------------===//
...
public:
  explicit StarcraftEUDDAGToDAGISel(StarcraftEUDTargetMachine &TM) : SelectionDAGISel(TM) {
    curr_func_ = nullptr;
  }

  StringRef getPassName() const override {
    return "StarcraftEUD DAG->DAG Pattern Instruction Selection";
  }
...
// ComplexPattern used on StarcraftEUD Load/Store instructions
bool StarcraftEUDDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
// ComplexPattern used on StarcraftEUD FI instruction
bool StarcraftEUDDAGToDAGISel::SelectFIAddr(SDValue Addr, SDValue &Base,
                                   SDValue &Offset) {
  SDLoc DL(Addr);
...
bool StarcraftEUDDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void StarcraftEUDDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::bpf_load_word: {
      ...
      SDValue R6Reg = CurDAG->getRegister(StarcraftEUD::R6, MVT::i64);
  ...
  case ISD::FrameIndex: {
    ...
    unsigned Opc = StarcraftEUD::MOV_rr;
    if (Node->hasOneUse()) {
...
void StarcraftEUDDAGToDAGISel::PreprocessLoad(SDNode *Node,
...
void StarcraftEUDDAGToDAGISel::PreprocessISelDAG() {
  ...
  //    StarcraftEUD LOAD already has zero extension.
  //
  //    This also solved a correctness issue.
  //    In StarcraftEUD socket-related program, e.g., __sk_buff->{data, data_end}
...
bool StarcraftEUDDAGToDAGISel::getConstantFieldValue(const GlobalAddressSDNode *Node,
...
bool StarcraftEUDDAGToDAGISel::fillGenericConstant(const DataLayout &DL,
                                          const Constant *CV,
                                          val_vec_type &Vals, uint64_t Offset) {
...
bool StarcraftEUDDAGToDAGISel::fillConstantDataArray(const DataLayout &DL,
...
bool StarcraftEUDDAGToDAGISel::fillConstantArray(const DataLayout &DL,
...
bool StarcraftEUDDAGToDAGISel::fillConstantStruct(const DataLayout &DL,
...
void StarcraftEUDDAGToDAGISel::PreprocessCopyToReg(SDNode *Node) {
  ...
  default:
    return;
  case 4:
    mem_load_op = StarcraftEUD::LDW;
    break;
  case 2:
    mem_load_op = StarcraftEUD::LDH;
    break;
  case 1:
    mem_load_op = StarcraftEUD::LDB;
    break;
  }
...
void StarcraftEUDDAGToDAGISel::PreprocessTrunc(SDNode *Node,
                                      SelectionDAG::allnodes_iterator &I) {
  ...
  default:
    return;
  case 0xFFFFFFFF:
    match_load_op = StarcraftEUD::LDW;
    break;
  case 0xFFFF:
    match_load_op = StarcraftEUD::LDH;
    break;
  case 0xFF:
    match_load_op = StarcraftEUD::LDB;
    break;
  }
...
bool StarcraftEUDDAGToDAGISel::checkLoadDef(unsigned DefReg, unsigned match_load_op) {
...
FunctionPass *llvm::createStarcraftEUDISelDag(StarcraftEUDTargetMachine &TM) {
  return new StarcraftEUDDAGToDAGISel(TM);
}
